...
int main()
{
	...
	string formula;
	cin >> formula;
	Map_ic proposition_set = getProposition(formula); // 逻辑变量
	int n = proposition_set.size();					  // 逻辑变量个数

	/* 输出变量个数和真值表表头 */
	cout << "该式子中的变量个数为：" << n << endl
		 << "输出真值表如下：" << endl;
	...
	/* 输出真值表 */
	int *m;
	m = (int *)malloc(sizeof(int) * pow2(n)); //该数组依次存放命题公式的各行(0或1)的运算结果的值
	for (int i = 0; i < pow2(n); i++)
	{
		Map_ii bina_set = toBinary(n, i);
...
Map_ic getProposition(string formula) //该函数返回所输入公式中的命题变项(不包括运算符)
{
	...
	for (unsigned int i = 0; i < formula.length(); i++)
	{
		...
		else if (!priority.count(c)) //既不是字母也不是运算符，exit
		{
